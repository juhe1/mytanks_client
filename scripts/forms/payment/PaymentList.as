package forms.payment
{
   ...
   public class PaymentList extends Sprite
   {
      ...
      public static function setBackground(_width:int) : BitmapData
      {
         ...
         data = new BitmapData(_width > 0 ? int(int(_width)) : int(int(1)),23,true,0);
         var subwidth:int = !!_withSMSText ? int(int(60)) : int(int((_width - 70) / 2));
      ...
      public function addItem(number:int, cost:Number, currency:String, crystals:int, smsText:String = "") : void
      {
         ...
         this.header.width = this.list.maxVerticalScrollPosition > 0 ? Number(Number(this._width - 15)) : Number(Number(this._width));
         StatLineBackgroundNormal.bg = new Bitmap(setBackground(this.list.maxVerticalScrollPosition > 0 ? int(int(this._width - 15)) : int(int(this._width))));
         StatLineBackgroundSelected.bg = new Bitmap(setBackground(this.list.maxVerticalScrollPosition > 0 ? int(int(this._width - 15)) : int(int(this._width))));
         this.dp.invalidate();
      }
