package utils.tweener
{
   ...
   public class TweenLite extends TweenCore
   {
      ...
      public function TweenLite(param1:Object, param2:Number, param3:Object)
      {
         ...
         this._overwrite = Number(param3.overwrite) <= -1 || !overwriteManager.enabled && param3.overwrite > 1 ? int(int(int(overwriteManager.mode))) : int(int(int(int(param3.overwrite))));
      ...
      protected function init() : void
      {
         ...
         for(_loc1_ in this.vars)
         {
            if(!(_loc1_ in _reservedProps && !(_loc1_ == "timeScale" && this.target is TweenCore)))
            {
               ...
               {
                  this.cachedPT1 = new PropTween(this.target,_loc1_,Number(this.target[_loc1_]),typeof this.vars[_loc1_] == "number" ? Number(Number(Number(Number(this.vars[_loc1_]) - this.target[_loc1_]))) : Number(Number(Number(Number(this.vars[_loc1_])))),_loc1_,false,this.cachedPT1);
