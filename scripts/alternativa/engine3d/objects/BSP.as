package alternativa.engine3d.objects
{
   ...
   public class BSP extends Object3D
   {
      ...
      {
         if(exludedObjects != null && exludedObjects[this])
         {
            return null;
         }
      ...
      private function intersectRayNode(node:Node, ox:Number, oy:Number, oz:Number, dx:Number, dy:Number, dz:Number) : RayIntersectionData
      {
         ...
         if(offset > 0)
         {
            ...
            dot = dx * node.normalX + dy * node.normalY + dz * node.normalZ;
            if(dot < 0)
            {
               time = -offset / dot;
               cx = ox + dx * time;
               ...
               while(true)
               {
                  if(f != null)
                  {
                     for(wrapper = f.wrapper; wrapper != null; wrapper = wrapper.next)
                     {
                        ...
                        acy = cy - a.y;
                        acz = cz - a.z;
                        if((acz * aby - acy * abz) * node.normalX + (acx * abz - acz * abx) * node.normalY + (acy * abx - acx * aby) * node.normalZ < 0)
                        {
                           break;
                        }
                     }
