package alternativa.physics.collision
{
   ...
   public class CollisionKdTree
   {
      ...
      private function splitNode(node:CollisionKdNode) : void
      {
         ...
         {
            boundBox = this.staticBoundBoxes[indices[i]];
            min = !!axisX ? Number(Number(boundBox.minX)) : (!!axisY ? Number(Number(boundBox.minY)) : Number(Number(boundBox.minZ)));
            max = !!axisX ? Number(Number(boundBox.maxX)) : (!!axisY ? Number(Number(boundBox.maxY)) : Number(Number(boundBox.maxZ)));
            if(max <= coordMax)
            {
               if(min < coordMin)
               {
